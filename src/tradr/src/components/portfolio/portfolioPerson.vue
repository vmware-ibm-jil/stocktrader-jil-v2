  ...
  <span>
    ...
    >
      <td class="table__cell">
        <router-link :to="{ name: 'Person', params: { user: row.owner }}">{{ row.owner }}</router-link>
      </td>
      <td class="table__cell">
      ...
      <td class="table__cell">
        <span>
          <hc-overflow class="overflow-sample" hc-placeholder="Select an option...">
            <portfolio-overflow-item
              hcModalPop="true"
              :hcModalTarget="'Buy ' + row.owner"
            >Buy Stocks</portfolio-overflow-item>
            <portfolio-overflow-item
              hcModalPop="true"
              :hcModalTarget="'Sell ' + row.owner"
            >Sell Stocks</portfolio-overflow-item>
            <portfolio-overflow-item
              hcHasDivider="true"
              hcModalPop="true"
              :hcModalTarget="'Statement ' + row.owner"
            >Download Statement</portfolio-overflow-item>
            <portfolio-overflow-item
              hcHasDivider="true"
              hcModalPop="true"
              :hcModalTarget="'Delete ' + row.owner"
            >Delete Portfolio</portfolio-overflow-item>
          </hc-overflow>
        </span>
      </td>
      <action-modal
        action="buy"
        :portfolio="row"
        :hcName="'Buy ' + row.owner"
        :hcTitle="'Buy stocks for '+ row.owner"
      ></action-modal>
      <action-modal
        action="sell"
        :portfolio="row"
        :hcName="'Sell ' + row.owner"
        :hc-title="'Sell stocks for '+ row.owner"
      ></action-modal>
      <action-modal
        action="statement"
        :portfolio="row"
        :hcName="'Statement ' + row.owner"
        :hc-title="'Statement for '+ row.owner"
      ></action-modal>
      <delete-portfolio-modal
        :owner="row.owner"
        :hcName="'Delete ' + row.owner"
        :hcTitle="'Delete '+ row.owner + '\'s Portfolio'"
      ></delete-portfolio-modal>
    </tr>
  </span>
</template>
