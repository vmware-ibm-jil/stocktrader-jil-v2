...
@RequestScoped // enable interceptors like @Transactional (note you need a WEB-INF/beans.xml in
				// your war)
/**
 * This version stores the Portfolios via JPA to DB2 (or whatever JDBC provider
 * is defined in your server.xml).
 */
public class PortfolioService extends Application {
	...
	private MqMessagePulisher messagePulisher = MqMessagePulisher.getInstance();
	...
	static {
		...
		if ((urlFromEnv != null) && !urlFromEnv.isEmpty()) {
			...
			logger.info(
					"Stock Quote URL not found from env var from config map, so defaulting to value in jvm.options: "
		...
		if ((urlFromEnv != null) && !urlFromEnv.isEmpty()) {
			...
			logger.info(
					"Trade History URL not found from env var from config map, so defaulting to value in jvm.options: "
							+ System.getProperty(mpUrlPropName));
		}
		...
		if ((urlFromEnv != null) && !urlFromEnv.isEmpty()) {
			...
			logger.info("ODM URL not found from env var from config map, so defaulting to value in jvm.options: "
					+ System.getProperty(mpUrlPropName));
		}
		...
		if ((urlFromEnv != null) && !urlFromEnv.isEmpty()) {
			...
			logger.info("Watson URL not found from env var from config map, so defaulting to value in jvm.options: "
					+ System.getProperty(mpUrlPropName));
		}
	}
...
//	@RolesAllowed({"StockTrader", "StockViewer"}) //Couldn't get this to work; had to do it through the web.xml instead :(
	...
	@Transactional
	// @RolesAllowed({"StockTrader"}) //Couldn't get this to work; had to do it
	// through the web.xml instead :(
	public Portfolio createPortfolio(@PathParam("owner") String owner) throws SQLException {
		...
		if (owner != null) {
			...
			// total=0.0, loyalty="Basic", balance=50.0, commissions=0.0, free=0,
			// sentiment="Unknown", nextCommission=9.99
			portfolio = new Portfolio(owner, 0.0, "Basic", 50.0, 0.0, 0, "Unknown", 9.99);

			logger.fine("Running following SQL: INSERT INTO Portfolio VALUES ('" + owner
					+ "', 0.0, 'Basic', 50.0, 0.0, 0, 'Unknown')");
			...
			messagePulisher.sendMessage(owner, "Create Portfolio",
...
//	@RolesAllowed({"StockTrader", "StockViewer"}) //Couldn't get this to work; had to do it through the web.xml instead :(
	public Portfolio getPortfolio(@PathParam("owner") String owner, @Context HttpServletRequest request)
		...
		if (portfolio != null) {
			...
			for (Stock stock : results) {
				...
				try {
					...
					// TODO - is it OK to update rows (not adding or deleting) in the Stock table
					// while iterating over its contents?
				...
				if (price != -1) // -1 is the marker for not being able to get the stock quote. But don't
			...
			logger.warning("No portfolio found for " + owner); // shouldn't get here; an exception with a 404 should be
																// thrown instead
		}
	...
	public String getPortfolioReturns(@PathParam("owner") String owner, @Context HttpServletRequest request)
		...
		try {
			...
			logger.info(
					"Unable to invoke TradeHistory.  This is an optional microservice and the following exception is expected if it is not deployed");
			utilities.logException(t);
		}
		return result;
	}
...
//	@RolesAllowed({"StockTrader"}) //Couldn't get this to work; had to do it through the web.xml instead :(
	public Portfolio updatePortfolio(@PathParam("owner") String owner, @QueryParam("symbol") String symbol,
			@QueryParam("shares") int shares, @Context HttpServletRequest request) throws IOException, SQLException {
		...
		if (!results.isEmpty()) { // row exists
			...
			if (newShares > 0) {
				logger.fine("Running following SQL: UPDATE Stock SET shares = " + newShares + ", commission = "
						+ newCommission + " WHERE owner = '" + owner + "' AND symbol = '" + symbol + "'");
				stock.setShares(newShares);
				stock.setCommission(newCommission);
				// getPortfolio will fill in the price, date and total
			} else {
				logger.fine("Running following SQL: DELETE FROM Stock WHERE owner = '" + owner + "' AND symbol = '"
						+ symbol + "'");
				stockDAO.deleteStock(stock);
			}
		} else {
			logger.fine("Running following SQL: INSERT INTO Stock (owner, symbol, shares, commission) VALUES ('" + owner
					+ "', '" + symbol + "', " + shares + ", " + commission + ")");
			stockDAO.createStock(stock);
			// getPortfolio will fill in the price, date and total
		}

		// getPortfolio will fill in the overall total and loyalty, and commit or
		// rollback the transaction
		logger.info("Refreshing portfolio for " + owner);
		portfolio = getPortfolio(owner, request);
		messagePulisher.sendMessage(owner, "Update Portfolio", "Symbol=" + symbol + ", Shares=" + shares);
...
//	@RolesAllowed({"StockTrader"}) //Couldn't get this to work; had to do it through the web.xml instead :(
	public Portfolio deletePortfolio(@PathParam("owner") String owner) throws SQLException {
		...
		messagePulisher.sendMessage(owner, "Deleted Portfolio", "Portfolio for owner deleted");
	...
	@Transactional
//	@RolesAllowed({"StockTrader"}) //Couldn't get this to work; had to do it through the web.xml instead :(
	public Feedback submitFeedback(@PathParam("owner") String owner, WatsonInput input)
			throws IOException, SQLException {
